...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    define(['fecha'], factory);
  } else if (typeof exports === 'object') {
    module.exports = factory(require('fecha'));
  } else {
    root.returnExports = factory(root.fecha);
  }
})(this, function(fecha, undefined) {
  ...
  var BTimeAgo = (function() {
    ...
    var _tsArray          = [],
        ...
        _option    = {
          ...
          static:         false,
          intervals: [
            // @interval:   [min, max, midnight-check]: min <= duration < max
            //                min: minisecond,
            //                max: minisecond,
            //                midnight-check: bool
            // @type:       fixed|relative|absolute|custom
    ...
    var _convertInterval = function(ts, option, type, unit, callback) {
      ...
      switch (type) {
        ...
        case 'relative':
          ...
          var p = ((-1 <= d && d <= 1) || option.shortUnit) ? '' : 's';
    ...
    BTimeAgo.print = function(timestamp, now) {
      BTimeAgo.now(now);
      return _compose(timestamp, _option);
    };
    ...
    BTimeAgo.fn = BTimeAgo.prototype = {
      ...
      init: function(timestamp, option, callback, skip) {
        ...
        this.option = _spread(_option, option);
