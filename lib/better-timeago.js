...
(function(root, factory) {
  ...
  var BTimeAgo = (function() {
    ...
    var _convertInterval = function(props, type, unit, callback) {
      ...
      switch (type) {
        case 'fixed':
          return option.shortUnit ? locale.units[unit][1] : locale.units[unit][0];
        case 'absolute':
        ...
        case 'custom':
          return callback(ts, now, option, locale);
      }
    }
    ...
    var _convert = function(props) {
      ...
      for (var i = 0; i < option.intervals.length; i++) {
        ...
        if (item.interval[0] <= duration && duration < item.interval[1]) {
          if (item.interval[2] && ts < startOf.today) {
            return _convertInterval(props, item.type[1], item.unit, item.callback);
          }
          return _convertInterval(props, item.type[0], item.unit, item.callback);
        }
      }
    };
    ...
    var _parseStartOf = function(now) {
      ...
      startOf.yesterday = new Date(_startOf.today - DAY);
      startOf.thisWeek = new Date(now - _getLocaleDay(now)*DAY);
      startOf.lastWeek = new Date(_startOf.thisWeek - WEEK);
